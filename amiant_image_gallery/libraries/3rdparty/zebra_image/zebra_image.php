...
class Zebra_Image
{
    ...
    function watermark($png_watermark_file, $alpha_level = 100)
    {
		$alpha_level	/= 100;	# convert 0-100 (%) alpha to decimal

		$watermark_img_obj = @imagecreatefrompng($png_watermark_file);

		$result = $this->_create_from_source();

		$main_img_obj_w	= imagesx($this->source_identifier);
		$main_img_obj_h	= imagesy($this->source_identifier);
        ...
        if ($result !== false) {
            ...
            // prepare the target image
            $target_identifier = $this->_prepare_image($main_img_obj_w, $main_img_obj_h, -1);
			...
			for( $y = 0; $y < $main_img_obj_h; $y++ ) {
				for( $x = 0; $x < $main_img_obj_w; $x++ ) {
					...
					$main_rgb = imagecolorsforindex( $this->source_identifier, imagecolorat( $this->source_identifier, $x, $y ) );
					...
					if (	$watermark_x >= 0 && $watermark_x < $watermark_img_obj_w &&
						...
						$return_color	= $this->_get_image_color( $target_identifier, $avg_red, $avg_green, $avg_blue );

					# if we're not dealing with an average color here, then let's just copy over the main color
					} else {
						$return_color	= imagecolorat( $this->source_identifier, $x, $y );
					} # END if watermark

					# draw the appropriate color onto the return image
					imagesetpixel($target_identifier, $x, $y, $return_color );
